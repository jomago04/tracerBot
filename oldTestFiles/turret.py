...
def main(stdscr):
    ...
    try:       
        while True:
            ...
            if key == ord('s'):
                control_motor(Motor2, 'forward', 1000, 0.000005)
            elif key == ord('w'):
                control_motor(Motor2, 'backward', 1000, 0.000005)
            elif key == ord('d'):
                control_motor(Motor1, 'backward', 100, 0.000005)
            elif key == ord('a'):
                control_motor(Motor1, 'forward', 10000, 0.000005)
            else:
                Motor1.Stop()
                Motor2.Stop()
